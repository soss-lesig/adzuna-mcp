...
3. Issue a public HTTPS URL (something like `https://adzuna-mcp-production.up.railway.app`).
4. Wait for you to set environment variables in the dashboard.

Set **both** `ADZUNA_APP_ID` and `ADZUNA_APP_KEY` in the Railway dashboard under your service's Variables tab. Optionally set `ALLOWED_ORIGINS` if you want to call the server from somewhere other than `claude.ai` or `localhost`.

Once the variables are set, Railway will redeploy automatically and the URL will start accepting requests. Add `https://your-url.up.railway.app/mcp` as a custom MCP connector in Claude.ai web (Settings → Connectors → Add custom).
...
```bash
docker build -t adzuna-mcp .
docker run -p 3000:3000 -e ADZUNA_APP_ID=your-id -e ADZUNA_APP_KEY=your-key adzuna-mcp
```
...
```bash
git clone https://github.com/soss-lesig/adzuna-mcp.git
cd adzuna-mcp
npm install
cp .env.example .env
# edit .env to set ADZUNA_APP_ID and ADZUNA_APP_KEY
...
Both `npm run` scripts use Node's `--env-file=.env` flag (no dotenv runtime dependency) and will fail loudly if `.env` is missing or either `ADZUNA_APP_ID` or `ADZUNA_APP_KEY` is unset. The path is resolved relative to the process's current working directory, so run the scripts from the repo root, or pass an absolute path (e.g. `--env-file=/srv/adzuna-mcp/.env`) when invoking from systemd, cron, or any other launcher with its own `cwd`.
